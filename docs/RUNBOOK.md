...
All four actions can be run locally via their runners in the `runners/` directory. By default they use aio Files (remote Azure Blob Storage), which requires the runtime credentials in `.env` and only works from within Adobe I/O Runtime.

### LOCAL_FS mode

Set `LOCAL_FS=true` to redirect all file I/O to `./local-data/` on disk instead of aio Files. This is useful for:
- Inspecting the products JSON generated by `fetch-all-products`
- Running the full pipeline locally without aio storage credentials
- Debugging rendering output without deploying
- Bypassing the App Builder action timeout limits (e.g. `fetch-all-products` has a 180s remote limit)

```bash
# Fetch all products → ./local-data/check-product-changes/default-products.json
LOCAL_FS=true node runners/run-fetch-all-products.js

# Check for product changes, render PDPs → ./local-data/public/pdps/...
LOCAL_FS=true node runners/run-product-changes-checker.js

# Render all category pages → ./local-data/public/plps/...
LOCAL_FS=true node runners/run-render-all-categories.js

# Clean up stale markup files from local-data/
LOCAL_FS=true node runners/run-mark-up-clean-up.js
```

Files are written under `./local-data/` mirroring the same path structure as remote storage:

```
local-data/
  check-product-changes/
    default-products.json   ← product list written by fetch-all-products
    default.csv             ← PDP poller state (SKU → lastRenderedAt,hash)
  render-all-categories/
    default.csv             ← PLP poller state (category → lastRenderedAt,hash)
  public/
    pdps/
      ...                   ← rendered PDP HTML files
    plps/
      ...                   ← rendered PLP HTML files
```

Note: in `LOCAL_FS` mode the "running" mutex is skipped, so concurrent local runs are not prevented.
